...
client.on('ready', async () => {
  ...
  client.user.setActivity(`Spadex Botu`)
});
client.on("message", async(message) =>{
  ...
  if (command == prefix + 'setstaff'){
    console.log(args)
    if (!message.member.hasPermission('ADMINISTRATOR')) return message.lineReply(`:x: Bu Komutu Kullanmaya Yetkin Yok`);
    if (args.length != 2) return message.lineReply({ embed: { description: `Lütfen Önce Admin Rolünün İdsini Sonra Yardımcı Rolünün idnisi Giriniz`, color: 0x5865F2 } })
    if (message.mentions.roles.length < 2 && !Number(args[0]) && !Number(args[1])) return message.lineReply({ embed: { description: `Lütfen Önce Admin Rolünün İdsini Sonra Yardımcı Rolünün idnisi Giriniz `, color: 0x5865F2 } })
...
client.on('clickButton', async (button) => {
  if (button.id === "oyb"){
    ...
    button.guild.channels.create(`ticket-${button.clicker.user.username}`, {
      ...
      if (button.id === "gyb"){
        await button.reply.send(`Biletiniz işleniyor. Lütfen bekle`, true)
        await db.set(`counts_${button.message.id}_${button.message.guild.id}`, `+`, 1)
        let count = await db.get(`counts_${button.message.id}_${button.message.guild.id}`)
        let channel;
        await button.clicker.fetch();
